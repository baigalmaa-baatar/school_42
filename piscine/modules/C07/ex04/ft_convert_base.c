...
int		ft_from_x_to_dec(char *base, char *input, long long *output)
{
	int base_size;
	int i;
	int digit;
	...
	i = 0;
	*output = 0;
	while ((digit = ft_char_at(input[i], base)) != -1)
	{
...
char	*ft_from_dec_to_x(char *base, long long input, int negative)
{
	int		base_size;
	int		i;
	char	*output;
